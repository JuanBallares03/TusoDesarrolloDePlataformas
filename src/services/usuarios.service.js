...
exports.findById = async (id_usuario) => {
    const [rows] = await db.execute('SELECT * FROM usuarios WHERE id = ?', [id_usuario]);
    return rows[0];
};
...
exports.update = async (id_usuario, updatedUser) => {
    const [result] = await db.execute(
        'UPDATE usuarios SET Nombre1 = ?, Nombre2 = ?, Apellido1 = ?, Apellido2 = ?, Correo_electronico = ?, Telefono = ?, Fecha_nac = ?, Sexo = ?, Ocupacion = ?, Puntos_xp = ? WHERE id = ?',
...
exports.remove = async (id_usuario) => {
    const [result] = await db.execute('DELETE FROM usuarios WHERE id = ?', [id_usuario]);
    return result.affectedRows > 0;
};